...
cv::Mat FootPrint::calcOpenPoseMask(std::vector<cv::Point2f> pts, cv::Size imSize){
...
//            capture.open( _projects_path + "/marker_calibration" + VIDEO_TYPE);
...
void FootPrint::getHomographyMatrix(){
    ...
    if(CHECKER_BOARD_CALIBRATION){
        ...
        obtainCheckerBoardPoints(imagePoints, scalePoints, W, H, SCALE, checkerBoardImage, TARGET_AREA_WIDTH * MtoMM,
        ...
        warpH = cv::findHomography(imagePoints, scalePoints);
        cv::warpPerspective(backGroundImage, overViewImage, warpH, cv::Size(400, 200));
...
//    cv::circle(overViewImage, cv::Point2f(200,100), 2, cv::Scalar(255,0,0), 2);
